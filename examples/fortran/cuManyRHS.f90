program cuManyRHS
    use mpi
    use iso_c_binding
    use cuFortranInterface   ! cuPTDMAPlanMany 관련 C 바인딩 인터페이스
    use fortranInterface     ! PTDMAPlanMany 관련 CPU 인터페이스
    implicit none
    ...
    ! ===== GPU 메모리 할당 및 복사 =====
    call cuda_malloc(a_d, nx*8_8)  ! double=8 bytes
    call cuda_malloc(b_d, nx*8_8)
    call cuda_malloc(c_d, nx*8_8)
