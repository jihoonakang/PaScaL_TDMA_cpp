...
def generate_2d_rhs(nx: int, ny: int, tdma_type: str):
    """
    Generate the right-hand side (RHS) matrix and reference solution for a 2D tridiagonal system.
...
def main(nx: int, ny: int, type_str: str):
    """
    Main function for solving multiple tridiagonal systems in 2D using MPI and PaScaL_TDMA.

    Each MPI process is assigned a subset of rows, and each row is solved independently
    using the TDMA solver.
