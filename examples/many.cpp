...
int main(int argc, char** argv) {
    ...
    if (argc != 3)
        throw std::runtime_error("Usage: many_example <nx> <ny>");
...
// Solve in x-direction
    ...
    collectSolution(d, d_sub, dom, topo);

    if (is_root) {
        double norm2 = 0.0;
        for (int i = 0; i < nx * ny; i++)
            norm2 += std::pow(d.getVector()[i] - x.getVector()[i], 2);
        std::cout << "Avg. RMS error = " << sqrt(norm2 / nx / ny)<< std::endl;
    }
...
void collectSolution(dimArray<double>& d, const dimArray<double>& d_sub,
    ...
    if (rank == 0)
        d_blk_recv.resize(nxy);

    std::vector<double> d_blk_send(n_sub);
    for (int i = 0; i < nx_sub; i++)
        for (int j = 0; j < ny_sub; j++)
            d_blk_send[j + i * ny_sub] = d_sub(i, j);

    MPI_Gatherv(d_blk_send.data(), n_sub, MPI_DOUBLE,
                d_blk_recv.data(), cnt_all.data(), disp_all.data(), MPI_DOUBLE,
                0, MPI_COMM_WORLD);
